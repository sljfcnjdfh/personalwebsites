;(function($){
	var comm_data = {
		...
		mark_str : "<div class='fixed-top' id='iwgc_dialog_bg' style='position:fixed; left:0; background:#000;bottom:auto;top:0;z-index:999;'></div>",
		win:$(window),
		doc:$(document)
	};
	...
	$.iwgcDialog = function (opt){
		...
		if($.iwgcDialog.dialog_default.cover){
			if(temp_cover.length==0){
				...
				temp_cover = $("#iwgc_dialog_bg");
				temp_cover.css({height:comm_data.doc.height(),width:comm_data.win.width(),opacity:$.iwgcDialog.dialog_default.opacity});
				if(comm_data.ie6){
		...
		if(temp_id.length==1){
			temp_id.show().css({
				...
				"left":(comm_data.win.width()-temp_id.outerWidth())/2,
				"zIndex":"99999"
			});
			if(comm_data.ie6){
	...
	$.extend($.iwgcDialog,{
		...
		resize_dialog : function(){
			temp_cover.css({height:comm_data.doc.height(),width:comm_data.win.width()});
			if(comm_data.ie6){
				...
				temp_id.css({"top":(comm_data.win.height()-temp_id.height())/2+"px","left":(comm_data.win.width()-temp_id.outerWidth())/2});
			}
		}
	});
